...
const RecipeListItem = ({ recipe, animationStyles }) => {
  ...
  const { image, usedIngredientCount, missedIngredientCount, title, usedIngredients, missedIngredients } = recipe;
  ...
  return (
    <animated.div style={{ ...animationStyles, ...styles.container }} onClick={() => setFlipped(!flipped)}>
      <AnimatedCard interactive style={{ ...flipBack, ...styles.card, zIndex: flipped ? 0 : 2 }}>
        ...
        <div style={styles.bottomSection}>
          <span>
            {`You have ${Math.round(
              (usedIngredientCount / (usedIngredientCount + missedIngredientCount)) * 100
            )}% of the ingredients!`}
          </span>
          <Button
          ...
          <Link to={`/recipes/${recipe.id}`}>
            <Button text="View Recipe" style={{ ...styles.viewButton, marginTop: 10 }} outlined />
          </Link>
        </div>
      </AnimatedCard>
      <AnimatedCard interactive style={{ ...flipForward, ...styles.card }}>
        ...
        <div style={styles.ingredientsContainer}>
          <div style={styles.usedText}>{usedIngredients.map((used) => capitalize(used.name)).join(", ")}</div>
          <div style={styles.missedText}>{missedIngredients.map((missed) => capitalize(missed.name)).join(", ")}</div>
        </div>
      </AnimatedCard>
    </animated.div>
  );
