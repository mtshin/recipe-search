  ...
  return (
    <div className="home__container">
      ...
      >
        ...
        <span style={styles.subtitleText}>Make something delicious with what you have at home!</span>
        <FormGroup helperText="Press comma or Enter to add ingredients" style={styles.inputContainer}>
          <TagInput
            ...
            rightElement={<Button onClick={handleClearText} icon="cross" minimal />}
            large
          />
        </FormGroup>
        <Button
      ...
      <div
        className={cx("result__container", {
          _hasResults: results.length > 0
        })}
      >
        {trail.map((animationStyles, index) => (
          <RecipeListItem key={results[index].id} recipe={results[index]} animationStyles={animationStyles} />
        ))}
      </div>
    </div>
  );
